...
from dataloaders.coco_id_idx_map import coco_id_idx_map
...
ex.add_config('./configs/coco80.yaml')
...
class MSCOCO17(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.num_things == 80:
            _coco_id_idx_map = np.vectorize(lambda x: coco_id_idx_map[x])
        elif self.num_things == 12:
            raise NotImplementedError
        else:
            raise NotImplementedError
...
def train_coco(cfg, model, model_init_weights, optimizer, data_loader, history, device, epoch, epoch_iter, _log):
    ...
    if not epoch < bootstrapping_start_epoch and epoch % teacher_update_interval.item() == 0:
        _log.info(f"Epoch {epoch:2d}, update teacher and reboot student, set epoch_iter and lr to 0")
        epoch_iter = 0
    ...
    for index, sample in enumerate(data_loader):
        ...
        losses = model(images, return_loss=True, label=label_cat, pseudo_labels=pseudo_label_things,
                       ...
                       augment=True, epoch=epoch_iter,cur_epo=epoch)
        loss_seg = losses['loss_seg'].mean()               
        ...
        assert len(w) == 4
        loss = w[0] * loss_seg + w[1] * loss_cat + w[2] * loss_uncertainty + w[3] * loss_cls_emb
        if loss > 0:
            loss.backward()
            ...
            losses_seg.update(loss_seg.detach().item())
        ...
        _log.info(f"train epoch: [{epoch}][{index + 1:4d}/{len(data_loader):4d}]\t"
                  ...
                  f"Loss(seg/cat/unc/emb/all): {losses_seg.val:.4f}/{losses_cat.val:.4f}/{losses_uncertainty.val:.4f}/{losses_cls_emb.val:.4f}/{losses_all.val:.4f} "
                  f"({losses_seg.avg:.4f}/{losses_cat.avg:.4f}/{losses_uncertainty.avg:.4f}/{losses_cls_emb.avg:.4f}/{losses_all.avg:.4f})")

    _log.info(f"* train epoch: [{epoch}]\t"
              f"loss(seg/cat/unc/emb/all): {losses_seg.avg:.4f}/{losses_cat.avg:.4f}/{losses_uncertainty.avg:.4f}/{losses_cls_emb.avg:.4f}/{losses_all.avg:.4f}")
...
def main(_run, _log):
    ...
    if cfg.eval_only:
        ...
        for epoch in range(cfg.num_epochs):
            ...
            sampler = DistributedSampler(train_dataset_coco, shuffle=True) if torch.cuda.device_count() > 1 else None
