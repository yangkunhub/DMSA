...
class Segmenter(nn.Module):
    ...
    def forward(self, x, return_loss=False, label=None, pseudo_labels=None, bootstrapping=False, augment=False, epoch=None, cur_epo=None):
        '''
        x:      N x 3 x H x W
        label:  N x H x W
        '''
...
#        print("cur_epo", cur_epo)
        if self.fix_encoder:
            with torch.no_grad():
                y, attn = self.encoder.forward_feat_attn(x,cur_epo = cur_epo)
        else:
            y, attn = self.encoder.forward_feat_attn(x,cur_epo = cur_epo)
        ...
        if return_loss:
            ...
            if bootstrapping:
                with torch.no_grad():
                    self.decoder_teacher.eval()
                    self.encoder_teacher.eval()
                    y_teacher, _ = self.encoder_teacher.forward_feat_attn(x,cur_epo=cur_epo)
            ...
            #------------------------先下采样减少计算量---------------------------------------------------------------------------
#            x2 = F.interpolate(x, size=(pseudo_labels.shape[2], pseudo_labels.shape[3]), mode='bilinear', align_corners=True)
            #--------------------------------------------------------------------------------------------------------------------
            
            loss_seg, loss_cat, loss_uncertainty, bootstrapped_pseudo_labels = \
                things_bootstrap_loss(masks_cls, pseudo_label=pseudo_labels,
                                      pseudo_label_teacher=pseudo_labels_teacher, epoch=epoch, images=None)
