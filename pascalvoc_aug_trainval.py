...
ex.add_config('./configs/pascalvocaug.yaml')
...
class PascalVOC(Dataset):
    def __init__(self,
        ...
        self.newepoch=0  #定义来捕获当前获取的epoch轮数
...
def eval_pascalvoc(cfg, model, data_loader, history, device, epoch, exp_ckpt_dir, _log, ):
    ...
    for index, sample in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            probs = model(images) #, cur_epo=epoch
...
def main(_run, _log):
    ...
    pseudo_label_train_save_dir = os.path.join(cfg.dataset.root_dir_pseudo_label, f'pseudo_label_pascalvoc_AUG')
    ...
    if cfg.eval_only:
        ...
        for epoch in range(cfg.num_epochs):
            ...
            if (epoch + 1) % cfg.eval_interval == 0:
                with torch.no_grad():
                    eval_pascalvoc(cfg, model, val_loader_pascalvoc, history, device, epoch, exp_ckpt_dir )
